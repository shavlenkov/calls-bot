...
bot.on('message', async (ctx) => {
  ...
  if (ctx.message.chat.type === "private") {
    if (regExpValidateTime.test(text_message) || regExpValidateTime2.test(text_message) && stateMsg === 'time') {
      ...
      await bot.telegram.sendPoll(chat.id, `${ctx.message.chat.username} хочет организовать созвон ${callDay} на ${time_message} по времени ${timeZone}`, optionsPool, { is_anonymous: false });
      callDay = 'сегодня';
      chat = {};
      ...
      if (chat) {
        ...
        stateMsg = 'time';
        ctx.reply('Во сколько ты хочешь организовать созвон?(формат ввода: XX.XX, XX,XX, XX:XX, XX XX или XX по (город), например(по Киеву)');
      } else {
        stateMsg = 'pool';
